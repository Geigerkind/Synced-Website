...
class EditMedia{
	...
	private function commit(){
		if($this->validUser() AND $this->validValues())
			$this->db->query('UPDATE media_stream SET title = "'.$this->formatValue($this->title).'", descr = "'.$this->formatText($this->text).'" WHERE id ='.$this->db->query('SELECT id FROM media_media WHERE vmid ='.$this->vmid)->fetch()->id);
		$this->goHome();
	}
